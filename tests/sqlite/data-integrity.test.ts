  ...
  describe('Fuzzy deduplication', () => {
    it('findDuplicateObservation catches near-identical narratives via fuzzy match', () => {
      ...
      const obs1 = createObservationInput({
        title: 'Nvidia GTC Demo',
        narrative: 'Completed the first successful dry-run of the robotic hand demo.',
      });
      ...
      const contentHash = computeObservationContentHash(memId, 'Nvidia GTC Demo', 'Completed the first successful dry run of the robotic hand demo.');
      const existing = findDuplicateObservation(db, contentHash, now + 1000, 'Nvidia GTC Demo', 'Completed the first successful dry run of the robotic hand demo.');
